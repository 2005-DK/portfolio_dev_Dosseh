...
@section('title', 'Ajouter une compétence - Admin Dashboard')
...
<div class="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
    ...
    <div class="sticky top-0 z-40 border-b border-purple-500/30 bg-slate-900/80 backdrop-blur-lg">
        <div class="px-6 py-4">
            <h1 class="text-3xl font-bold text-white">Ajouter une Compétence</h1>
        </div>
    </div>
                ...
                <form action="{{ route('admin.skills.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" placeholder="Laravel, Vue.js, React..." required class="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-purple-500 transition-colors @error('name') border-red-500 @enderror">
                            @error('name')
                        ...
                        <div>
                            ...
                            <select name="category" required class="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white focus:outline-none focus:border-purple-500 transition-colors @error('category') border-red-500 @enderror">
                                ...
                                <option value="database" @selected(old('category') == 'database')>Base de données</option>
                                <option value="devops" @selected(old('category') == 'devops')>DevOps</option>
                                <option value="tools" @selected(old('category') == 'tools')>Outils</option>
                                <option value="other" @selected(old('category') == 'other')>Autres</option>
                            </select>
                            @error('category')
                            <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="number" name="proficiency" value="{{ old('proficiency', 80) }}" min="0" max="100" class="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white focus:outline-none focus:border-purple-500 transition-colors">
                            <div class="mt-2 w-full h-2 bg-slate-700 rounded-full overflow-hidden">
                                <div class="h-full bg-gradient-to-r from-purple-500 to-pink-500 rounded-full" style="width: {{ old('proficiency', 80) }}%"></div>
                            </div>
                            <p class="text-xs text-gray-400 mt-1">{{ old('proficiency', 80) }}% - Ajustez avec le curseur</p>
                        </div>

                        <!-- Icon -->
                        <div>
                            <label class="block text-sm font-semibold text-white mb-2">Icon (Emoji ou Unicode)</label>
                            <input type="text" name="icon" value="{{ old('icon') }}" placeholder="🚀, ⚡, 💎..." class="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-purple-500 transition-colors text-2xl text-center">

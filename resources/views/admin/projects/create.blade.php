...
@section('title', 'Créer Projet - Admin Dashboard')
...
<div class="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
    ...
    <div class="sticky top-0 z-40 border-b border-purple-500/30 bg-slate-900/80 backdrop-blur-lg">
        <div class="px-6 py-4">
            <h1 class="text-3xl font-bold text-white">Créer un nouveau projet</h1>
        </div>
    </div>
            ...
            <div class="relative bg-slate-800 border border-slate-700 rounded-xl p-8">
                <form action="{{ route('admin.projects.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" required class="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-purple-500 transition-colors @error('title') border-red-500 @enderror" placeholder="Mon super projet...">
                        @error('title')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="technologies" value="{{ is_array(old('technologies')) ? implode(', ', old('technologies')) : old('technologies') }}" placeholder="Laravel, React, Tailwind..." class="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-purple-500 transition-colors">
                        ...
                        <div>
                            ...
                            <input type="url" name="url" value="{{ old('url') }}" placeholder="https://example.com" class="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-purple-500 transition-colors">
                        </div>

                        <!-- GitHub URL -->
                        <div>
                            <label class="block text-sm font-semibold text-white mb-2">URL GitHub</label>
                            <input type="url" name="github_url" value="{{ old('github_url') }}" placeholder="https://github.com/..." class="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-purple-500 transition-colors">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="is_published" required class="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white focus:outline-none focus:border-purple-500 transition-colors">
                                <option value="1" {{ old('is_published', 1) == 1 ? 'selected' : '' }}>Public</option>
                                <option value="0" {{ old('is_published', 1) == 0 ? 'selected' : '' }}>Privé</option>
                            </select>
                        </div>
                    </div>
...
function previewImage(input) {
    ...
    reader.onload = function(e) {
        preview.innerHTML = `
            <div class="mt-4">
                <p class="text-sm text-gray-300 mb-3">Aperçu de l'image (${(file.size / 1024).toFixed(1)}KB):</p>
                <div class="bg-slate-900 rounded-lg p-2 inline-block border border-slate-700">
                    <div class="relative group">
                        ...
                        <button type="button" onclick="clearImage()" class="absolute top-1 right-1 bg-red-500 hover:bg-red-600 text-white p-2 rounded-full shadow-lg transition-all opacity-0 group-hover:opacity-100">
